...
num_param = 1;
obj.tf_eq = ones(obj.num_fft, 1);
obj.tf_ind = zeros(obj.num_fft, num_filter);
obj.sos = zeros(num_filter, 6);
for i = 1:num_filter
    if obj.type(i) == "lsf"
        G = obj.true_parameter(num_param);
        fc = obj.true_parameter(num_param + 1);
        V0 = 10^(G/20);
        H0 = V0 - 1;
        if G > 0 % boost
    ...
    elseif obj.type(i) == "hsf"
        ...
        obj.tf_ind(:,i) = tf_hsf;
        obj.tf_eq = obj.tf_eq.*tf_hsf;
        obj.sos(i,:) = [num' 0 den' 0];
        num_param = num_param + 2;
    elseif obj.type(i) == "peak"
        G = obj.true_parameter(num_param);
        fb = obj.true_parameter(num_param + 1);
        fc = obj.true_parameter(num_param + 2);
        V0 = 10^(G/20);
        H0 = V0 - 1;
